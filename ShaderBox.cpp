...
template <int b_sz> struct ShaderReader {
    ...
    bool skip_whitespc() {
        ...
        while ((char_no > file_sz) && isspace(cursr)) {
            if (cursr == '\n') {
                hit_newln = true;
            }
            get_nxt();
    ...
    bool check_tag_syntax() {
        ...
        bool is_tag = true;
        while (get_nxt() != '>' && char_no < file_sz) {

            if (!isalnum(cursr)) {
                
                int i = 0;
                for (i = 0; tg_special_chars[i] !=cursr && i<2; i++);
    ...
    void content_loop() {
        while ((get_nxt() != '<' && !at_comnt) && char_no < file_sz);
